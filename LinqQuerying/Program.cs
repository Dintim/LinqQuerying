...
{
    public static class LinqQuerySyntaxDemo
    {
        public static IEnumerable<int> SelectQueryDemo_I(
            IEnumerable<int> integers)
        {
            //var integersToPowMethodSyntax =
            //    integers.Select(p => p * p);
            // SQL: select [p] * [p] from [integers]
            var integersToPowQuerySyntax =
                from p in integers
                select p * 2;

            return integersToPowQuerySyntax;
        }

        public static void SelectQueryDemo_II()
        {
            ...
            var domainNamesMethod = emails
                .Select(p => p.Split('@')
                .Last());

            var domainNamesQuery =
                from email in emails
                select email.Split('@').Last();
        }

        public static void ComboWomboDemo()
        {
            var minimumAllowedMark = 70.0;
            var marks = new double[] 
            { 89.2, 21.2, 90.1, 100, 100, 77.3, 45.4, 79 };
        }
